...
fn initialize_forest(_rng: &mut Mt19937GenRand64) -> Vec<Vec<usize>> {
    let threshold = (u64::MAX / 10) * 6; // Seuil pour déterminer la probabilité d'avoir un arbre
    (0..SIZE)
        .map(|_| {
            (0..SIZE)
                .map(|_| if _rng.next_u64() < threshold { TREE } else { EMPTY })
                .collect()
        })
        .collect()
}
...
fn simulate_fire(forest: &mut Vec<Vec<usize>>, _rng: &mut Mt19937GenRand64) {
...
fn main() {
    let mut _rng = Mt19937GenRand64::new(4136);
